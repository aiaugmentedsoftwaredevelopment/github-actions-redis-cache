...
export class GzipNativeHandler implements CompressionHandler {
  readonly format = CompressionFormat.GZIP;
  readonly priority = 100; // Higher than shell-based gzip (25), lower than tar-gzip-native (200)
  ...
  async compress(
    ...
    core.debug(
      `[gzip-native] Creating archive with ${paths.length} paths at level ${compressionLevel}`
    );
    core.debug(`[gzip-native] Output: ${outputFile}`);
    core.debug(
      '[gzip-native] Note: Creating tar.gz archive (tar + gzip compression)'
    );
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        output.on('finish', () => {
          ...
          core.debug(
            `[gzip-native] Archive created: ${formatBytes(totalBytes)} (${totalFiles} files)`
          );
          resolve();
        });
  ...
  private async addToTar(
    ...
    try {
      ...
      if (stats.isSymbolicLink()) {
        ...
        pack.entry(
          ...
          (err) => {
            if (err)
              core.warning(
                `[gzip-native] Failed to add symlink ${relativePath}: ${err.message}`
              );
          }
        );
        ...
        pack.entry(
          ...
          (err) => {
            if (err)
              core.warning(
                `[gzip-native] Failed to add directory ${relativePath}: ${err.message}`
              );
          }
        );
      ...
      core.warning(
        `[gzip-native] Failed to add ${relativePath}: ${errorMsg}`
      );
    }
  }

  async extract(archivePath: string, targetDir: string): Promise<void> {
    core.debug(`[gzip-native] Extracting archive: ${archivePath}`);
    core.debug(`[gzip-native] Target directory: ${targetDir}`);

    // Verify archive exists and get size
    try {
      const stats = fs.statSync(archivePath);
      core.debug(`[gzip-native] Archive size: ${formatBytes(stats.size)}`);
    } catch (error) {
    ...
    extract.on('entry', async (header, stream, next) => {
      ...
      core.debug(`[gzip-native] Extracting: ${header.name}`);
      ...
      try {
        if (header.type === 'directory') {
          ...
          output.on('error', (err) => {
            core.error(
              `[gzip-native] Failed to write file ${header.name}: ${err.message}`
            );
            next(err);
          });
          ...
          // Skip unknown types
          core.debug(
            `[gzip-native] Skipping unsupported entry type: ${header.type}`
          );
          stream.resume();
          next();
        }
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        core.error(
          `[gzip-native] Failed to extract ${header.name}: ${errorMsg}`
        );
        stream.resume();
        next();
      }
    ...
    await new Promise<void>((resolve, reject) => {
      extract.on('finish', () => {
        core.debug(
          `[gzip-native] Extraction completed: ${extractedFiles} files`
        );
        resolve();
      });
