/**
 * Tests for native LZ4 compression handler
 */

import * as fs from 'fs';
import * as path from 'path';
import * as os from 'os';
import {Lz4NativeHandler} from '../lz4-native';
import {CompressionFormat} from '../../types';

describe('Lz4NativeHandler', () => {
  let handler: Lz4NativeHandler;
  let testDir: string;
  let archivePath: string;

  beforeEach(() => {
    handler = new Lz4NativeHandler();
    // Create temporary test directory
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lz4-native-test-'));
    archivePath = path.join(testDir, 'test-archive.lz4');
  });
  ...
  describe('detect()', () => {
    test('should always return true (pure JavaScript implementation)', async () => {
  ...
  describe('compress() and extract()', () => {
    ...
    test('should compress and extract multiple files', async () => {
      // Create test files
      const files = [
        {name: 'file1.txt', content: 'LZ4 Content 1'},
        {name: 'file2.txt', content: 'LZ4 Content 2'},
        {name: 'file3.txt', content: 'LZ4 Content 3'},
      ];
    ...
    test('should compress and extract a directory structure', async () => {
      ...
      // Extract to new directory
      const extractDir = path.join(os.tmpdir(), 'lz4-extract-' + Date.now());
      fs.mkdirSync(extractDir);
      await handler.extract(archivePath, extractDir);
    ...
    test('should handle compressible data efficiently', async () => {
      ...
      const extractedFile = path.join(extractDir, 'compressible.txt');
      const extractedContent = fs.readFileSync(extractedFile, 'utf8');
    ...
    test('should handle large files', async () => {
      // Create a larger file (10KB - lz4js pure JS implementation is slow for larger sizes)
      const testFile = path.join(testDir, 'large.bin');
      const buffer = Buffer.alloc(10 * 1024, 'L');
      ...
      expect(extractedStats.size).toBe(10 * 1024);
    });
    ...
    test('should throw error when extracting non-existent archive', async () => {
      const nonExistentArchive = path.join(testDir, 'nonexistent.lz4');
      const extractDir = path.join(testDir, 'extract');
      fs.mkdirSync(extractDir);
    ...
    test('should create target directory if it does not exist', async () => {
      // Create test file
      const testFile = path.join(testDir, 'test.txt');
      fs.writeFileSync(testFile, 'Test content for LZ4');
    ...
    test('should handle compression of relative paths', async () => {
      ...
      try {
        ...
        // Create file
        fs.writeFileSync('relative-lz4.txt', 'Relative path for LZ4');

        // Compress with relative path
        const archive = path.join(testDir, 'relative.lz4');
        await handler.compress(['relative-lz4.txt'], archive, 6);
        ...
        // Verify
        const extractedFile = path.join(extractDir, 'relative-lz4.txt');
        expect(fs.existsSync(extractedFile)).toBe(true);
        expect(fs.readFileSync(extractedFile, 'utf8')).toBe(
          'Relative path for LZ4'
        );
      } finally {
    ...
    test('should handle multiple directory levels', async () => {
      ...
      // Extract
      const extractDir = path.join(os.tmpdir(), 'lz4-deep-' + Date.now());
      fs.mkdirSync(extractDir);
      await handler.extract(archivePath, extractDir);
    ...
    test('should handle mixed content types', async () => {
      ...
      fs.writeFileSync(binaryFile, Buffer.from([0xDE, 0xAD, 0xBE, 0xEF]));
      fs.mkdirSync(subDir);
      ...
      const baseName = path.basename(testDir);
      expect(
        fs.readFileSync(path.join(extractDir, baseName, 'text.txt'), 'utf8')
      ).toBe('Text content');
      expect(
        fs.readFileSync(path.join(extractDir, baseName, 'binary.bin'))
      ).toEqual(Buffer.from([0xDE, 0xAD, 0xBE, 0xEF]));
      expect(
        fs.readFileSync(
          path.join(extractDir, baseName, 'subdir', 'nested.txt'),
          'utf8'
        )
      ).toBe('Nested');
    });
